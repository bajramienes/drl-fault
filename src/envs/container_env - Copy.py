# src/envs/container_env.py — FINAL VERSION (100% compatible)
...
REAL_EVALUATION = True   # ← 100% real Docker during evaluation (CSV data)
...
class HybridContainerEnv(gym.Env):
    def __init__(self, agent_name="PPO", phase="early", seed=42, is_training=True):
        ...
        if REAL_EVALUATION and not is_training:
            try:
                ...
                print("[WARN] Docker not available → using realistic simulation for this episode")
                self.docker_client = None
    ...
    def step(self, action):
        ...
        if hasattr(action, "__len__"):
            ...
            act_val = {0: -0.8, 1: 0.0, 2: 0.8}.get(int(action), 0.0)
        ...
        info = {
            "action_taken": "scale" if act_val > 0.3 else "migrate" if act_val < -0.3 else "noop",
